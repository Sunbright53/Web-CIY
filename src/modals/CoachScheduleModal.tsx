...
export const CoachScheduleModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  const [selectedDay, setSelectedDay] = useState(DAYS[new Date().getDay() - 1] || "Saturday");
  const [rawSchedule, setRawSchedule] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(false);

  // ✅ แก้ไข: เพิ่ม selectedCoach เข้าไปเพื่อให้โหลดข้อมูลใหม่เมื่อกดเปลี่ยนโค้ช
  useEffect(() => {
    if (isOpen) {
      const load = async () => {
        setIsLoading(true);
  ...
  }, [isOpen, selectedCoach]); // <-- เพิ่ม selectedCoach ตรงนี้

  // ✅ ปรับปรุงการกรองข้อมูลให้รองรับตัวเล็ก/ใหญ่ และ Master Slots
  const scheduleByTime = useMemo(() => {
    if (!rawSchedule) return {};

    const filtered = rawSchedule.filter(item => {
      const itemCoach = String(item.coach || "").toLowerCase();
      const targetSearch = selectedCoach.replace("Coach ", "").toLowerCase();
      // กรองตามชื่อโค้ช (ตัวย่อ 3 ตัว) และวันที่เลือก
      return itemCoach.includes(targetSearch) && item.day === selectedDay;
    });
    
    // จัดกลุ่มตามเวลา (เช่น "11:00 - 12:30")
    ...
    // เรียงลำดับเวลาจากเช้าไปเย็น (Optional)
              ...
              <div key={time} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[...Array(4)].map((_, i) => (
                    <div 
                      key={i} 
                      className={`h-24 rounded-2xl flex flex-col items-center justify-center border-2 border-dashed transition-all
