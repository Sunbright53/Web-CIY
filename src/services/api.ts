...
/** ✅ ปรับปรุง: เพิ่มนักเรียนใหม่พร้อมจองตารางเรียนประจำ (Fix Schedule) */
/** ✅ ปรับปรุง: เพิ่มนักเรียนใหม่พร้อมจองตารางเรียนประจำ (Fix Schedule) 
 * รองรับหัวตาราง: Day, Time, Coach, Coder_number, Coder_name
 */
export async function submitStudent(url: string, data: AddStudentForm): Promise<void> {
  const res = await postForm(url, {
    ...
    // --- ข้อมูลพื้นฐานสำหรับฐานข้อมูล Students ---
    ...
    // --- ✅ ข้อมูลที่ส่งไปให้ CIY Booking System (Fix_date_coder) ---
    // ใช้คีย์ 'fix_...' เพื่อให้ Apps Script ดึงค่าไปลงคอลัมน์ได้แม่นยำ
    fix_day: data.day ?? '',     // บันทึกลงคอลัมน์ Day
    fix_time: data.time ?? '',   // บันทึกลงคอลัมน์ Time
    fix_coach: data.coach ?? '', // บันทึกลงคอลัมน์ Coach
    
    // หมายเหตุ: Apps Script ฝั่ง Booking ของคุณจะใช้ coder_id เป็น Coder_number 
    // และใช้ nickname เป็น Coder_name ตามที่คุณระบุไว้ในสคริปต์
  });
